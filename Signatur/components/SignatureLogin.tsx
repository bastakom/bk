...
export default function SignatureLogin() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch('/api/signature-auth/login', {
      method: 'POST',
  ...
  return (
    <main className="min-h-screen bg-[#f3f3f1] text-[15px] leading-[1.45] text-[#0b0b0b] antialiased">
      ...
      <div className="mx-auto grid min-h-[calc(100vh-55px)] max-w-[1200px] place-items-center px-[18px] py-12">
        <form onSubmit={handleSubmit} className="w-full max-w-[420px] border-[1.5px] border-[#dededa] bg-white">
          <div className="border-b border-[#dededa] p-5">
            ...
            <p className="mt-2 text-sm text-[#70706c]">
              Signaturgeneratorn är låst med ett gemensamt lösenord.
            </p>
          </div>
