...
function localizedStoryHref(fullSlug: string | undefined, locale: string) {
...
function filmCaseImageAlt(item: any) {
  return (
    item?.content?.videoimage?.alt ||
    item?.content?.videoimage?.name ||
    `${item.name} - filmcase`
  );
}
            ...
            >
              {props.map((item: any) => {
                return (
                  <Link
                    key={item.uuid}
                    href={localizedStoryHref(item.full_slug, locale)}
